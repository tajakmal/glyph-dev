    ...
    <div className="bg-zinc-900 border-b border-zinc-800">
      ...
      <div className="h-12 flex items-center justify-between px-2 sm:px-4">
        ...
        <div className="flex items-center gap-1 sm:gap-3 min-w-0 flex-1">
          ...
          {title && (
            <span className="text-zinc-300 text-sm font-medium truncate" title={title}>
              {title}
            </span>
          )}
        </div>
        ...
        <div className="hidden sm:flex items-center gap-2 flex-shrink-0 mx-2">
          <button
            onClick={zoomOut}
        ...
        {/* Right: Speed Read, Bookmark, Page indicator, Theme */}
        <div className="flex items-center gap-1 sm:gap-3 flex-shrink-0">
          ...
          <span className="sm:hidden text-zinc-400 text-xs tabular-nums">
      ...
      <div className="sm:hidden px-3 pb-1.5 flex items-center gap-2">
        <button
          onClick={zoomOut}
          disabled={zoom <= VALIDATION.MIN_ZOOM}
          className="p-1.5 text-zinc-400 hover:text-zinc-100 rounded transition-colors disabled:opacity-50"
          aria-label="Zoom out"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 12H4" />
        ...
        <div className="flex-1 h-1 bg-zinc-800 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, ((zoom - VALIDATION.MIN_ZOOM) / (VALIDATION.MAX_ZOOM - VALIDATION.MIN_ZOOM)) * 100)}%` }}
          />
        </div>
        <button
          onClick={zoomIn}
        ...
        <span className="text-zinc-500 text-xs tabular-nums w-10 text-right">{zoomPercent}%</span>
      </div>
    </div>
  );
}
